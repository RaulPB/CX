...
{!! Form::close() !!}
	<table class="table table-striped">
		...
		?>
			...
			<td>{{date('d-m-Y', strtotime($venta -> fecha_limite))}}</td> 
			<td>{{$venta -> total_venta}}</td>
			<td>{!!link_to_route('venta.edit', $title = 'Verificar', $parameters = $venta->idventa, $attributes = ['class'=>'btn btn-info btn-lg btn-block'])!!}</td>
		</tbody>
			@endforeach
	</table>
{!!$ventas->render()!!}
